exports.run = async (client, message, args, level) => { // eslint-disable-line no-unused-vars
	...
	if (!time) return message.reply('**Use:** | .lembrete [tempo]  [Mensagem] ');
	if (!reminder) return message.reply('**Use:** | .lembrete [tempo]  [Mensagem] ');
	...
	setTimeout(function () {
		message.reply(`<a:sino:809882202866319442> Você me pediu para te lembrar de: \`${reminder}\` `);
	}, parseInt(timems));
