...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!discrims.length) return message.reply("Eu não achei niguém com esse discriminador!");
    ...
    sourcebin.create([{
      ...
      .setTitle(`Discriminador - ${dis[0]}`)
      .setDescription(`${content}\n\n**[Link da lista!](${src.url})**`)
      .setFooter(`Achei ${discrims.length} user(s) com esse discriminador!`, client.user.displayAvatarURL())
      
      message.channel.send(embed);
    })
  .catch(e => {
         message.channel.send(`Falha no sistema! Tente novamente!`)
   });
